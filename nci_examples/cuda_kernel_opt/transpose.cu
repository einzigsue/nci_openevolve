...
int main(int argc, char* argv[]) {
    ...
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " <matrix_file> <matrix_size>" << std::endl;
        return 1;
    }

    int height = std::stoi(argv[2]);  // nrows
    int width = std::stoi(argv[2]);   // ncols
    ...
    for (int ii = 0; ii < numElements; ++ii) {
        if (!(file >> h_in[ii])) {
            std::cerr << "Error reading matrix file. Check format and size." << std::endl;
            return 1;
        }
    }
    file.close(); 
